...
class AlienFleet:
    alien_exploding_images = [pg.image.load(f'images/Ship1_Explosion/Ship1_Explosion_{n+3}.png') for n in range(8)]
    ...
    alien_images = [[pg.transform.rotozoom(pg.image.load(f'images/Ship{m}_img_{n}.png'), 0, 0.80) for n in range(2)] for m in range(3)]
    ufo_imgs = [pg.transform.rotozoom(pg.image.load(f'images/ship3_img_{n}.png'), 0, 0.75) for n in range(2)]
    ...
    def create_alien(self, row, col):
        x = self.alien_w * (1.1 * col + 1)
        y = self.alien_h * (1.1 * row + 1)
